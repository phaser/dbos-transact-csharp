...
[Collection("Postgres")]
public sealed class PostgresSchedulesDaoTests : IClassFixture<PostgresFixture>
{
    private const string Schema = "dbos_sched_pg_test";

    private readonly PostgresFixture _fixture;

    public PostgresSchedulesDaoTests(PostgresFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    private async Task<SystemDatabase> CreateAsync()
    {
        ...
        // Clear any leftover rows from prior tests in the shared schema.
        await using var cleanupConn = new NpgsqlConnection(_fixture.ConnectionString);
        await cleanupConn.OpenAsync();
        await using var cmd = cleanupConn.CreateCommand();
        cmd.CommandText = $"DELETE FROM \"{Schema}\".workflow_schedules";
    ...
    public async Task ListSchedules_FilterByPrefix_EscapesWildcards()
    {
        ...
        // Literal "%" must match only the one literal name, not act as wildcard.
    ...
    public async Task ApplySchedules_ReplacesExisting_ResetsStateAndId()
    {
        ...
        // Re-apply with status Paused — but Apply must reset to Active and clear last_fired_at and assign a new id.
